...
namespace ThirdParty.QPathFinder.Script.Followers
{
    ...
    public static class PathFollowerUtility
    {
        ...
        private static System.Func<List<System.Object>, int, Vector3> GetPositionFromNodeOrVector = delegate (List<System.Object> list, int index) { return (list[index] is Vector3 ? (Vector3)list[index] : ((Node)list[index]).Position); };
        private static System.Func<List<System.Object>, int, Node> GetNodeFromNodeOrVector = delegate (List<System.Object> list, int index) { return (list[index] is Node ? (Node)list[index] : null); };

        #endregion PRIVATE AND INTERNAL
    }
}